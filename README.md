# Catan Map Generator

Modern seed-based Catan board generator.

## Start
```bash
python3 -m http.server 3000
```
Open: **http://localhost:3000**

## Features
- 19 hexagons (3-4-5-4-3 layout)
- **Classic Catan icons**: Detailed illustrations (trees, bricks, sheep, wheat, mountains, cactus)
- Seed system (reproducible boards)
- **Classic preset**: Original Catan rules (6/8 can't touch, same resources can't touch)
- **Custom preset**: Full control over placement rules
- Modern dark theme
- Mobile responsive

## Presets
- **Classic**: Balanced, original game rules (recommended)
- **Custom**: Customize all placement rules

## Cache
If background looks black: Hard refresh `Cmd+Shift+R` (Mac) or `Ctrl+Shift+R` (Windows)

MIT License

